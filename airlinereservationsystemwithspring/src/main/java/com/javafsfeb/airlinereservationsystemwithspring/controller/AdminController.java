...
public class AdminController {
  ...
  private AdminService adminService;
	...
	@GetMapping(path="/showReservations",
			produces = {MediaType.APPLICATION_JSON_VALUE,MediaType.APPLICATION_XML_VALUE})
	public AirlineResponce showReservations() {
		List<BookReservationInfoBean> recordList = adminService.showReservations();
		AirlineResponce response = new AirlineResponce();
		
		if(recordList != null && !recordList.isEmpty()) {
			response.setShowReservations(recordList);
		} else {
			response.setError(true);
			response.setMessage("No reservations Yet");
		}
		return response;
	}
