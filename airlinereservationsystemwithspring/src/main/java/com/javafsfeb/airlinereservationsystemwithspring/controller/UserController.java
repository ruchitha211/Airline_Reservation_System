...
public class UserController {
	...
	@GetMapping(path="/searchByDestinaton/{destination}",
			produces= MediaType.APPLICATION_JSON_VALUE)
	public AirlineResponce booking(String source, String destination) {
		FlightDetailsInfoBean flightBySourceandDestination=userService.booking(source,destination);
		AirlineResponce responce= new AirlineResponce();
		if(flightBySourceandDestination!=null) {
			responce.setMessage("Flights  Avaliable are....");
			responce.setFlightDetails(flightBySourceandDestination);
		}else {
			responce.setError(true);
		}
		return responce;
	...
	@GetMapping(path="/searchBySourceDestinaton",
			produces= MediaType.APPLICATION_JSON_VALUE)
	...
	public AirlineResponce getFlightDetails() {
		List<FlightDetailsInfoBean> recordList = userService.getFlightDetails();
		AirlineResponce response = new AirlineResponce();
	...
	@PostMapping(path="/toBookFlight",
			consumes= {MediaType.APPLICATION_JSON_VALUE,MediaType.APPLICATION_XML_VALUE},
			produces= {MediaType.APPLICATION_JSON_VALUE,MediaType.APPLICATION_XML_VALUE})
	public AirlineResponce bookflight(@RequestBody BookReservationInfoBean flightbooking) {
		BookReservationInfoBean isAdded = userService.bookflight(flightbooking);
		AirlineResponce response = new AirlineResponce();
		if(isAdded!=null) {
			...
			response.setError(true);
			response.setMessage("Unable to Book  A flight Please try Again!!!");
		}
		return response;
	}
